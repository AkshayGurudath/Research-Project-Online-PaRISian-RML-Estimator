...
class PaRIS_estimator(object):
    ...
    def setup(self, model, nPart, nBackDraws, data):
        self.model = model
        self.nPart = nPart
    ...
    def run(self):
        ...
        for tt in range(1, self.data.T):
         ...
         #                                                                  self.nPart)) / self.nBackDraws
                ...
                tStatN = tStatN + (tStat[:, bInd] + self.model.test(xPartN,xPart[bInd], self.data.yt[tt],self.data.yt[tt-1]))[0] / self.nBackDraws
            ...
            zeta1 = np.sum(self.model.test(xPartN,xPart[bInd], self.data.yt[tt],self.data.yt[tt-1])[1], axis=1)/ self.nPart
