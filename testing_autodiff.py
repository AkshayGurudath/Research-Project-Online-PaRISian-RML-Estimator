...
class stochVol(object):
    ...
    @tf.function
    def grad_tests(self,xt,xtprev,yt,ytprev,diff):
        with tf.GradientTape(persistent=True) as tape:
            ...
            #y1=tf.math.log(a)
        c=tape.jacobian(a,diff)
        d=tape.jacobian(b,diff)
        return (c,d)
    ...
    def test(self,xt,xtprev,yt,ytprev):
       ...
       # a=np.zeros((self.nPar, nPart))
        c=self.grad_tests(xt,xtprev,yt,ytprev,diff)
        print(c)
...
def hello():
    ...
    m.par[2] = 1
    for i in range(10):    
        m.test([0.5,0.5,0.7],[0.6,0.6,0.6],[1,1,1],[1,1,1])
